...
def make_prediction(user_dict, restaurant, user,k,similarityThreshold):
    similar_dict = topMatches(user_dict, user, k, similarity=cosineSimilarity)
    numerator = 0.0
    denominator = 0.0
...
def cosineSimilarity(user_i, user_j, user_dict):
    commonItemList=[]
...
def topMatches(user_dict,user_to_predict,k,similarity=cosineSimilarity):
