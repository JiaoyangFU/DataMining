...
An critical parameter used to calculate predications is similarity function.Among many proposed similarity functions we choose Cosine Similarity and Pearson Correlation to give a detailed introduction. 
...
In Pearson Correlation model, it computes the statistical correlation between two user's common ratings to determine their similarity. The formula is given below indicating how to calculate the Cosine Similarity between user $u$ and $v$. 
...
\subsubsection{User-based CF-Experiment 1}
In this experiment, we used all training data and tried to predicate rating for each user on each item. Also cosine similarity was used to measure similarity. Both RMSE and MAE was used to measure differences between predicated values and true values.
...
K is the number of nearest neighbors chosen for each user when calculating the predicated ratings. we can see from above charts that both RMSE and MAE value is very high(larger than 1.0) when k is 20. But with the increasing of k, both RMSE and MAE decrease quickly. This is because when more meaningful similar users are taking into consideration when calculating  ratings, the accuracy thus is improved. 

\subsubsection{User-based CF-Experiment 2}
In this experiment, we predicated ratings for users in the extracted 20\% testing data. Also cosine similarity was used to measure similarity. Both RMSE and MAE was used to measure differences between predicated values and true values.
...
K is the number of nearest neighbors chosen for each user when calculating the predicated ratings. we can see from above charts that when k is 20,both RMSE and MAE reach their lowest value respectively.when k is lower than 20 or larger than 20, both RMSE and MAE increases. This is because if too many users with low similarity to current user are considered, the accuracy is negatively affected. Also, if not enough users with high similarity to current user are chosen, the accuracy cannot achieve the best.  

\subsubsection{User-based CF-Experiment 3}
In this experiment, we predicated ratings for users in the extracted 20\% testing data. Pearson correlation was used to measure similarity. Both RMSE and MAE was used to measure differences between predicated values and true values.
...
K is the number of nearest neighbors chosen for each user when calculating the predicated ratings. When comparing the result of item based prediction and user based prediction, we can see under the same given conditions user based prediction surpasses item based prediction. This is because pearson correlation is more suitable for calculating similarity for user based prediction. 
